...
instance Applicative (Reader r) where
  ...
  liftA2 :: (a -> b -> c) -> Reader r a -> Reader r b -> Reader r c
  liftA2 f reader1 reader2 = Reader (\r -> f (runReader reader1 r) (runReader reader2 r))

instance Monad (Reader r) where
  (>>=) :: Reader r a -> (a -> Reader r b) -> Reader r b
  reader >>= f = Reader (\r -> runReader (f (runReader reader r)) r)
...
processAccount account = do
  ...
  isMinBal <- checkMinimumBalance account
  return (newAccount, interest, isMinBal)